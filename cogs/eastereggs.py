...
class EasterEggs(commands.Cog):  # Creates a class for the cog that inherits from commands.Cog
	# this class is used to create a cog, which is a module that can be added to the bot

	def __init__(self, bot):  # this is a special method that is called when the cog is loaded
		self.bot = bot

	@commands.Cog.listener()  # Adds an event listener to the cog
	...
	@commands.command(name = "salty")  # Creates a prefixed command
	...
	@commands.command(name = "russianbias")  # Creates a prefixed command
	async def russianbias(self, ctx):
		image = "https://tinyurl.com/4e2x43nd"
		await ctx.channel.send(image)
		await ctx.message.delete()

	@commands.command(name = "scottsman")  # Creates a prefixed command
	...
	@commands.command(name = "posh")  # Creates a prefixed command
